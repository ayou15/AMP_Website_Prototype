...
//Check if an Sig exists in the DB
function sigExists($username)
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("SELECT *
		...
		$query = $mysqli->query( "SELECT * FROM ".$db_table_prefix."users  WHERE user_name = '$username' ") or die($mysql->error);
...
function fetchAllUsers()
{
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		FROM ".$db_table_prefix."users");
	$stmt->execute();
	$stmt->bind_result($id, $user, $display, $password, $email, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn, $sig);
	
	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'user_name' => $user, 'display_name' => $display, 'password' => $password, 'email' => $email, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn, 'sig' => $sig);
	}
	$stmt->close();
	return ($row);
}
